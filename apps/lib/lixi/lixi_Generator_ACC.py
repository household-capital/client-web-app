...
class LixiXMLGenerator:
    ...
    def __init__(self,lixiSettings, productionData,submissionID, applicationID, commentStr, targetFilename):
        ...
        #Package
        self.root = ElementTree.Element('Package',nsmap=self.LIXI_SETTINGS['ns_map_'],ProductionData=productionData,UniqueID=submissionID)
        ...
        self.accountCreationInstructions=ElementTree.SubElement(self.instructions, 'AccountCreationInstructions')
        self.submit=ElementTree.SubElement(self.accountCreationInstructions, 'Submit', IsAccountVariation="No",IsResubmission="No")

        #-Publisher
        self.publisher = ElementTree.SubElement(self.root, 'Publisher', CompanyName="HouseholdCapital", LIXICode='LIXILIXI',
                                                  Email='lendingservices@householdcapital.com', PublishedDateTime=self.__getTimeStamp())

        #-Recipient
        self.recipient = ElementTree.SubElement(self.root, 'Recipient', Description='AMAL',LIXICode='LIXILIXI')

        #-SchemaVersion
        self.schemaversion = ElementTree.SubElement(self.root, 'SchemaVersion', LIXITransactionType="ACC", LIXIVersion=self.LIXI_SETTINGS['LIXI_VERSION'])
    ...
    def populateElements(self, srcDict):
        ...
        try:
            self.address = ElementTree.SubElement(self.application,'Address',dict(AustralianPostCode=srcDict['Prop.Postcode__c'],AustralianState=srcDict['Prop.state'],
            ...
            #Comment
            self.detailedcomment=ElementTree.SubElement(self.application, 'DetailedComment',ContextDescription="Description" )
        ...
        try:
            ...
            #-Term
            self.rateComposition = ElementTree.SubElement(self.loanDetail, "RateComposition", dict(OriginatorMargin=str(srcDict["Loan.OriginatorMargin"])))
            #-Term
            self.loanTerm = ElementTree.SubElement(self.loanDetail, "Term", dict(InterestType="Variable", PaymentType="Interest Capitalised", PaymentTypeDuration="1", PaymentTypeUnits="Months", TotalTermDuration="480",
                                                                                 TotalTermType="Total Term", TotalTermUnits="Months", InterestTypeUnits="Months"))
        except:
                ...
                self.personapplicant = ElementTree.SubElement(self.application, "PersonApplicant",
                                                              dict(ApplicantType="Borrower", DateOfBirth=srcDict[
                                                                  "Brwr" + str(i + 1) + ".Birthdate__c"],
                                                                   ...
                                                                   MaritalStatus=srcDict["Brwr" + str(i + 1) + ".MaritalStatus"],
                                                                   PrimaryApplicant=srcDict["Brwr" + str(i + 1) + ".PrimaryApplicant"],
        ...
        try:
            ...
            #Real Estate Asset
            self.realEstateAsset=ElementTree.SubElement(self.application,'RealEstateAsset',dict(PrimaryPurpose='Owner Occupied',PrimaryUsage='Residential',
                                                             PropertyID="HHC-"+srcDict['Prop.Id'],
                                                             Remoteness =  srcDict['Prop.Remoteness'],
                                                             Status='Established',
            ...
            self.relatedCompany=ElementTree.SubElement(self.application, 'RelatedCompany', dict(CompanyName=srcDict['Prop.Valuer__c'],UniqueID=str(srcDict['Prop.Valuer__c']).replace(" ", "")))
            ...
            self.valuerContact = ElementTree.SubElement(self.relatedCompanyContact, "ContactPerson",
                                                               dict(Role="Valuer", x_ContactPerson="HHC-" + srcDict['Prop.Valuer_Name__c'].replace(" ","")))
        ...
        try:
            ...
            for i in range(int(srcDict["POA.Number"])):
                self.relatedPerson = ElementTree.SubElement(self.application, 'RelatedPerson',
                                                             dict(DateOfBirth=srcDict["POA" + str(i + 1) + ".Birthdate__c"],
                                                                  UniqueID="POA-"+srcDict['POA'+str(i+1)+".Id"]))

                self.relatedPersonContact=ElementTree.SubElement(self.relatedPerson,"Contact",dict(x_Address="POA-"+srcDict['POA'+str(i+1)+".Id"]))
        ...
        try:
            ...
            if srcDict['Loan.Distribution_Partner_Contact__c']!=None:
                ...
                self.Introducer = ElementTree.SubElement(self.salesChannel, 'Introducer',
                                                         dict(ContactName=srcDict['Dist.FirstName']+" "+srcDict['Dist.LastName'],
        ...
        try:
            ...
            self.FeePercentage = ElementTree.SubElement(self.Fee, 'Percentage', dict(Rate=str(srcDict['Opp.Establishment_Fee_Percent__c'])))
        except:
    ...
    def __getTimeStamp(self):
        ...
        return datetime.datetime.now().replace(tzinfo=datetime.timezone(offset=utc_offset)).replace(microsecond=0).isoformat()
